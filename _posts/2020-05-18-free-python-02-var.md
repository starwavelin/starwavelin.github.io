...
### 读取用户输入
我们所写的程序肯定希望能够贴近用户的需求。这就要求我们的程序能够从用户那里得到一些输入信息。
在Python的世界里，或许用户从键盘端的输入还是蛮简单的，只要用一个名叫`input()`的函数。

在Python解释器下，我们输入一下`input()`函数：
```
>>> input()
   [光标自动移到下一行]
```

在光标所在的一行，我们随便打打字，比如中文`我想说啥？我也不知道`
```
>>> input()
我想说啥？我也不知道  [在原来的光标所在行打下这些字]
'我想说啥？我也不知道'  [原光标的下方自动呈现出你，作为用户，打下的字]
```

`input()`函数里可以带上你希望用户看到的提示词，string类型的提示词。而`input()`函数的到的结果，也可以存到一个变量里，通过打印这个变量，你会发现`input()`函数的返回结果也是属于string类型。
```
>>> inp = input('Enter your name please:')
Enter your name please:代码笔记哥  [“代码笔记哥”为我，作为用户，输入的]
>>> print(inp)
代码笔记哥
```

那，假如我们要打出通过用户输入、得到一个可以用于数学计算的整数或者浮点类型，应该怎么办呢？从上面内容我们知道，如果我们只是通过`input()`函数，并且假定用户也输了一个数字，比如`12.75`，我们所得到的只是一个内容为"12.75"的string类型变量。好在，Python为我们准备了`int()`和`float()`函数，你可以把这两个函数翻译为“整数化”和“浮点数化”。顾名思义，它们可以把`input()`函数得出的结果（string型）转换为int型或float型。
...
### 代码注释
注释指示代码当中用于解释或提醒程序员的文字内容，不参与代码的执行。在Python中，代码注释有两种方法。

如果注释内容是短内容，可以使用`#`号，然后把你要写的注释写在后面。
```
# 华氏温标转为摄氏温标的计算方法
celcius = (fahrenheit - 32) * 5 / 9
```

如果注释内容是长内容，可以使用三个单引号`'''`号开始，再来三个单引号`'''`号结束的方式，把你要写的注释写在上下单引号之间。
```
'''
这是一段长注释，
下面这段代码
讲的是对冲基金中的风险分析的算法
'''
def risk_analysis(inp, risk_factor):
    # ...
    # ...
```

### 布置作业
好，讲到这里，你已经学完了Python小课第2课“变量、表达式和语句”的所有内容。


下面，请你做一点题目练练手吧。

第1题：  
写一个Python程序询问用户的名字，并问候用户。程序的样例输出如下:
```
告诉我你的名字：王小亮
你好，王小亮
```
...
第2题：算工资。  
写一个Python程序询问用户的工作时长和小时薪资，而后打印出用户得到的工资。程序的样例输出如下:
```
How many hours do you work: 40
What is your hourly rate: 35.85
Pay: 1,434
```

### 总结
总结一下。第二章的内容主要是让读者朋友们了解变量、表达式和语句这些Python程序的最简单的基石，并用Terminal打开Python解释器写一些简单的Python程序。  
